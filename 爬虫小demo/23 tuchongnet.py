...
class LBTuChongNet(object):
	def __init__(self):
		self.loginUrl = "https://stock.tuchong.com/accounts/login"
	...
	def get_crypt_password(self,message):
		...
		password = rsa.encrypt(message, self.key)
	...
	def login(self,username,password):
		...
		rsp = self.session.post(login_url, data = postdata, headers = self.headers)
		if str(rsp).find('200'):
			print("登陆成功！")
